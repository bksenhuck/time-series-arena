...
from ml.features.feature_engineering import FEATURE_COLS, build_single_row
...
def recursive_xgb_forecast(
    ...
    try:
        ...
        return _mean_fallback(series, horizon, residual_std)
...
def _mean_fallback(
    series: pd.Series, horizon: int, residual_std: float
) -> dict:
    """Fallback: repeat last 7-day mean when series_id is unseen."""
    mean_val = float(series.iloc[-7:].mean())
    ...
    return {
        "dates":  strftime_list(fcast_dates),
        "mean":   predictions,
        "lower":  [max(0.0, mean_val - w) for w in width],
        "upper":  [mean_val + w for w in width],
    }
