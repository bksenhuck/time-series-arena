...
def smooth(series: pd.Series, window: int = 7) -> pd.Series:
    """Apply a rolling-mean smoothing (used for visualisation only)."""
    return series.rolling(window, min_periods=1).mean()


def preprocess_series(series: pd.Series, fill: bool = True, clip_outliers: bool = True) -> pd.Series:
    if fill:
...
def preprocess_all(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for sid, group in df.groupby("series_id"):
        ...
        tmp["series_id"] = sid
        cleaned.append(tmp)
    return pd.concat(cleaned, ignore_index=True)[["date", "series_id", "views"]]
